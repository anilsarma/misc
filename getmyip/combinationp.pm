...
sub usage($)
{
    ...
    print STDERR "usage: $program_name <options>  (n1 n2 n3 n4 n5) \n";
    print STDERR "\tversion:\n";
    print STDERR "options:\n";
    print STDERR "       -samples <sample count>  total number of samples to use default unlimited\n";
...
foreach my $d(@l)
{
        ...
        my $average = $d->{'total'};
        my $key =  combinationp::print_data($d->{'data'}, undef);
        if( scalar(@compare)==5)
        {
           my $key2 =  combinationp::print_data(\@freq, undef);
           next if ($key ne $key2);
           print "Sample=(", combinationp::print_data(\@compare, undef), ") sum=" , sum(@compare), "\n";
        }
        if($actual>0)
        {
                $average /= $actual;
        ...
        print $d->{'outcomes'}, " p=", $d->{'p'}*100 , " tries=", $tries, " estimated=", $estimated, " acutal=", $actual, " total=", $d->{'total'}, " average=", $average, " data=",  combinationp::print_data($d->{'data'}, undef), "\n";
        foreach my $r(@records)
        {
                my @w = $r->white();
               print "\t", $r->date, " ", combinationp::print_data(\@w, undef), "sum=", sum(@w), "\n";
        }
}

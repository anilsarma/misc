...
function draw_array(arr) {
  ...
  console.log( $("#status").text);
   ...
   before = $("#app").css("background-color");
   $("#app").css("background-color","red");
   $("#app").css("animation","blinker 2s linear infinite");
   console.log(before);
   //console.log(hexToRgb(colourNameToHex("red")));
  if (before != $("#app").css("background-color") )
   {
        audio = new Audio();
        audio.src = "http://www.springfieldfiles.com/sounds/homer/mcrap.mp3"
        audio.play();
    }



  $("#status").html($("#status").html() + "<br/>" + arr['First Data']);
   $("#input").text(arr['First Data']);
  //text.value = "hello world " + arr;
  var canvas = $('#canvas')[0];
  ...
  //console.log(dirs(arr));
  //var img = ctx.getImageData(0, 0, arr.keys().length, arr[0].length);
